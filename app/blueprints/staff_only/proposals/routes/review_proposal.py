...
@proposals_group("/review/<int:proposal_id>", methods=["GET", "POST"])
def review_proposal(proposal_id):
    ...
    if request.method == "POST":
        change_requested = (
            True if request.form.get("change_requested") == "true" else False
        )
        ...
        if request.form.get("redirect_to"):
            ...
            return redirect(
                url_for(request.form.get("redirect_to"), proposal_id=proposal_id)
            )
        ...
        if vote_for:
            ...
            ProposalVotes.vote_against(
                proposal.proposal_id, session.get("account_id", 0)
            )
        else:
        ...
        flash("Proposal has been updated.")
        return redirect(
            url_for("staff_only.proposals.review_proposal", proposal_id=proposal_id)
        )
    ...
    return render_template(
        ...
        show_voting=True,
        vote_position=ProposalVotes.get_vote_position(
            proposal.proposal_id, session.get("account_id", 0)
        ),
    )
